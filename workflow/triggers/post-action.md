...
:::info{title="Подсказка"}
При запуске событий после действия через вызовы HTTP API необходимо обратить внимание на статус включения рабочего процесса и на то, соответствует ли конфигурация коллекции, в противном случае вызов может не пройти успешно или может возникнуть ошибка.
:::

Для рабочих процессов, локально привязанных к кнопке действия, можно вызвать их следующим образом (на примере кнопки создания коллекции `posts`):
