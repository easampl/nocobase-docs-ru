...
При выборе уже объявленной переменной потребуется также выбрать целевую переменную, то есть узел, где эта переменная была объявлена:
...
В последующих узлах после узла переменной вы можете использовать значение переменной, выбрав объявленную переменную из группы «Переменные узла». Например, в узле запроса использовать значение переменной в качестве условия запроса:
