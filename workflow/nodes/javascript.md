...
Узел JavaScript позволяет выполнять пользовательский серверный JavaScript в рамках рабочего процесса. В скрипте можно использовать переменные из предыдущих этапов рабочего процесса в качестве параметров, а возвращаемое значение скрипта может быть передано последующим узлам.

Скрипт выполняется на сервере приложения NocoBase и поддерживает большинство функций Node.js. Однако существуют некоторые отличия от нативной среды выполнения. Подробнее см. в разделе [Список возможностей](#список-возможностей).

## Создание узла

В интерфейсе настройки рабочего процесса нажмите кнопку («+»), чтобы добавить узел «JavaScript»:
...
Используются для передачи переменных или статических значений из контекста рабочего процесса в скрипт. `name` — это имя параметра, которое после передачи в скрипт становится именем переменной. `value` — это значение параметра, которое может быть переменной или константой.
...
Если скрипт завершится с ошибкой, он не вернет никакого значения, и результат узла будет заполнен сообщением об ошибке. Если последующие узлы используют переменную результата из узла JavaScript, к этому следует относиться с осторожностью.
:::
