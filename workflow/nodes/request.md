...
В интерфейсе настройки рабочего процесса нажмите кнопку («+») , чтобы добавить узел «HTTP-запрос»:
...
Заголовок запроса `Content-Type` настраивается через формат данных запроса. Нет необходимости заполнять его здесь, любое переопределение будет неэффективным.
:::

### Параметры запроса

Пары «ключ-значение» для секции параметров запроса. Значения могут использовать переменные из контекста рабочего процесса.
...
Результат ответа HTTP-запроса может быть разобран узлом [Сопоставление данных JSON](./json-query.md) для использования в последующих узлах.
...
Заголовки ответа представлены в формате JSON. Как заголовки, так и данные ответа все еще требуют разбора с помощью узла Сопоставление данные JSON перед использованием.
