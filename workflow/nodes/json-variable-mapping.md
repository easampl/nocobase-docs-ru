...
В интерфейсе конфигурации рабочего процесса нажмите кнопку («+»), чтобы добавить узел «Сопоставление JSON-переменных»:
...
Источником данных может быть результат вышестоящего узла или объект данных в контексте процесса. Обычно это неструктурированный объект данных, например, результат SQL-узла или узла HTTP-запроса.
...
Изначально JSON-структура может быть сложной, но после сопоставления  останеться только выбрать переменную для соответствующего пути.