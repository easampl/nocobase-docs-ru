...
В одноузловой среде плагины обычно закрывают задачи за счёт состояния в процессе, событий или задач. Однако в режиме кластера один и тот же плагин может одновременно работать на нескольких экземплярах, что приводит к типичным проблемам:

- **Согласованность состояния**: если конфигурация или данные времени выполнения хранятся только в памяти, их сложно синхронизировать между экземплярами, что приводит к «грязным» чтениям или дублирующимся выполнениям.
- **Планирование задач**: без явного механизма очередей и подтверждений длительные задачи могут одновременно выполняться несколькими экземплярами.
- **Состояния гонки**: операции изменения схемы или распределения ресурсов должны быть сериализованы, чтобы избежать конфликтов при конкурентной записи.

Ядро NocoBase предоставляет на уровне приложения набор интерфейсов промежуточного слоя, которые помогают плагинам переиспользовать единые возможности в кластерной среде. Ниже описаны практики и примеры для кэширования, синхронных сообщений, очереди сообщений и распределённых блокировок.
